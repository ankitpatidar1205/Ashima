          ...
          {sortedProducts?.length > 0 ? (
            sortedProducts.map((product) => (
             ...
             <span onClick={() => handleToggleStatus(product.id, product.status)} 
              className={`text-xs px-2 py-1 rounded cursor-pointer ${ product.status === "1" ? "bg-green-100 text-green-600"
              : "bg-yellow-100 text-yellow-600"}`}>  {product.status === "1" ? "Published" : "Draft"} </span></div>
