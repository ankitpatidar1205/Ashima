...
const TrendingWithCards = () => {
  ...
  activeTab === null
    ? true
    : course.category_id.toString() === categories[activeTab]?.id?.toString()
)
    ...
    .map((course, index) => (
      ...
      <div className="absolute bottom-0 left-0 w-full bg-gradient-to-t from-black/80 via-black/50 to-black/20 backdrop-blur-sm px-4 pb-4 pt-4 text-white tracking-wide min-h-[220px] flex flex-col justify-end">
        <h3 className="text-[25px] mb-1">{course.course_title}</h3>
        ...
        <div className="flex items-center justify-between text- gap-2 mt-3">
          {/* Learn More Button */}
          <button
            onClick={() => navigate(`/Cource-Detail/${course.id}`)}
      ...
      <div className="flex items-center justify-between w-full max-w-[1440px] mx-auto mt-2 px-8">
        <div>
          <button onClick={scrollLeft}   className="bg-[#00E0C6] text-[#000000] p-2 rounded-full hover:bg-[#00c5b0] transition">
            <FaArrowLeft className="h-3 w-3" />
          </button>
        </div>
        <div>
          <button  onClick={scrollRight}  className="bg-[#00E0C6] text-[#000000] p-2 rounded-full hover:bg-[#00c5b0] transition" >
